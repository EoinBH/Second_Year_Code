...
Rel compute_tv_pass_value(Tree *t) {
  Rel r(2);
    if((t->dtrs[0]->mother.cat == "was") && (t->dtrs[1]->mother.cat == "kissed")) {
      //Derive a relation from kissed with reversed tuples
      Rel r_reversed = (flip(kissed));
      return r_reversed; //RETURN RELATION WITH REVERSED TUPLE
    }
    else if((t->dtrs[0]->mother.cat == "was") && (t->dtrs[1]->mother.cat == "frozen")) {
        //Derive a relation from froze with reversed tuples
        Rel r_reversed = (flip(froze));
        return r_reversed; //RETURN RELATION WITH REVERSED TUPLE
    }
    else {
      sem_error(t);
...
Rel flip(Rel r) {
  //Invert the order in all the contained tuples
  for (int i = 0; i < r.tuples.size(); i++) {
    reverse(r.tuples[i].begin(), r.tuples[i].end());
  }
  //Return relation with reversed tuples
  return r;
}
