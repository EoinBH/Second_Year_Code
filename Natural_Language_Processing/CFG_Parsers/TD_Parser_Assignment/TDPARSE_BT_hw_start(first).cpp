...
bool TDPARSE_BT::accept(vector<string> key) {
  ...
  do {
	...
	extend_history(rulecount);
	cout << "2rulecount: " << rulecount << "\n";
	Category c = pda.pop();
	for (unsigned int i = rulecount; i < g.nrules; i++) {
	  cout << "in for statement";
	  if (g.rules[i].mother == c.cat) {
	    cout << "if statement";
	    r = g.rules[i];
    ...
    if (final_config() == true) {
      ...
      BT_ITEM obj = history.back();
      history.pop_back();
      rulecount = obj.rulecount + 1;
      cout << "\nrestored rulecount:" << rulecount << "\n";
      progress = obj.progress;
      PDA tmp_stack;
      for (unsigned int i = 0; i < (unsigned int)obj.pda.size(); i++) {
	tmp_stack.push(obj.pda.pop());
      }
      for (unsigned int i = 0; i < (unsigned int)tmp_stack.size(); i++) {
	pda.push(tmp_stack.pop());
      }
...
void TDPARSE_BT::extend_history(unsigned int rulecount) {
  ...
  if(later) {
    ...
    BT_ITEM obj;
    PDA tmp_stack;
    for (unsigned int i = 0; i < (unsigned int)pda.size(); i++) {
      tmp_stack.push(pda.pop());
    }
    for (unsigned int i = 0; i < (unsigned int)tmp_stack.size(); i++) {
      obj.pda.push(tmp_stack.pop());
    }
    obj.rulecount = rulecount;
    cout << "rulecount in obj: " << rulecount;
    obj.progress = progress;
    cout << "progress in obj: " << progress;
